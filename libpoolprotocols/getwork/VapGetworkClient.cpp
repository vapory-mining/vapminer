...
VapGetworkClient::VapGetworkClient(int worktimeout, unsigned farmRecheckPeriod)
  : PoolClient(),
...
void VapGetworkClient::connect()
{
        ...
        m_conn->HostNameType() == dev::UriHostNameType::Basic)
    {
        ...
        m_resolver.async_resolve(
            q, m_io_strand.wrap(boost::bind(&VapGetworkClient::handle_resolve, this,
...
void VapGetworkClient::disconnect()
{
    // Release session
...
void VapGetworkClient::begin_connect()
{
    if (!m_endpoints.empty())
    {
        ...
        m_socket.async_connect(
            m_endpoint, m_io_strand.wrap(boost::bind(&VapGetworkClient::handle_connect, this, _1)));
    }
    else
    {
...
void VapGetworkClient::handle_connect(const boost::system::error_code& ec)
{
            ...
            while (m_txQueue.pop(line))
            {
                if (line->size())
                {
                    ...
                    async_write(m_socket, m_request,
                        m_io_strand.wrap(boost::bind(&VapGetworkClient::handle_write, this,
...
void VapGetworkClient::handle_write(const boost::system::error_code& ec)
{
    if (!ec)
    {
        ...
        async_read(m_socket, m_response, boost::asio::transfer_all(),
            m_io_strand.wrap(boost::bind(&VapGetworkClient::handle_read, this,
...
void VapGetworkClient::handle_read(
...
void VapGetworkClient::handle_resolve(
...
void VapGetworkClient::processResponse(Json::Value& JRes) 
{
    ...
    // 9 as response for vap_submitHashrate
    ...
    if (_id == 0 || _id == 1)
    {
        ...
        if (!_isSuccess)
        {
            ...
            m_getwork_timer.async_wait(
                m_io_strand.wrap(boost::bind(&VapGetworkClient::getwork_timer_elapsed, this,
                    boost::asio::placeholders::error)));
        }
        else
        {
            ...
            {
                cwarn << "Missing data for vap_getWork request from " << m_conn->Host() << ":"
                      << toString(m_conn->Port());
            }
            else
            {
...
std::string VapGetworkClient::processError(Json::Value& JRes)
{
    std::string retVar;
...
void VapGetworkClient::send(Json::Value const& jReq)
{
    send(std::string(Json::writeString(m_jSwBuilder, jReq)));
}

void VapGetworkClient::send(std::string const& sReq) 
{
...
void VapGetworkClient::submitHashrate(uint64_t const& rate, string const& id)
{
...
void VapGetworkClient::submitSolution(const Solution& solution)
{
...
void VapGetworkClient::getwork_timer_elapsed(const boost::system::error_code& ec) 
{
